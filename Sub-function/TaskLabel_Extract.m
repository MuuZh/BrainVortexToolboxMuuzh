...
elseif flagTask == 4  % Motor Task
    TaskLabel_AllSubject_motor = [];
    for subject = 1:No_of_Subject
        foldername = [main_folder,'/Sample Data/Motor Task/Raw Data'];
        cd(foldername)
        name = dir([pwd]) ;
        foldername = [name(subject+2).folder,'/',name(subject+2).name,'/MNINonLinear/Results/tfMRI_MOTOR_LR/EVs'];
        cd(foldername)          
        cue = load('cue.txt') ;
        leftFoot = load('lf.txt') ;
        leftHand = load('lh.txt') ;
        rightFoot = load('rf.txt') ;
        rightHand = load('rh.txt') ;
        tongue = load('t.txt');




        leftFoot_restructure = [leftFoot(:,1) leftFoot(:,1)+leftFoot(:,2) 1.*ones(size(leftFoot,1),1)];  % Left Foot == 1 
        leftHand_restructure = [leftHand(:,1) leftHand(:,1)+leftHand(:,2) 2.*ones(size(leftHand,1),1)]; %Left Hand = 2
        rightFoot_restructure = [rightFoot(:,1) rightFoot(:,1)+rightFoot(:,2) 3.*ones(size(rightFoot,1),1)]; %Right Foot == 3
        rightHand_restructure = [rightHand(:,1) rightHand(:,1)+rightHand(:,2) 4.*ones(size(rightHand,1),1)]; %Right Hand == 4
        tongue_restructure = [tongue(:,1) tongue(:,1)+tongue(:,2) 5.*ones(size(tongue,1),1)]; %Tongue == 5
        
        
        TaskLabel_1sub = [leftFoot_restructure;leftHand_restructure;rightFoot_restructure...
            ;rightHand_restructure;tongue_restructure];
        TaskLabel_1sub_timestep = round([TaskLabel_1sub(:,1)./0.72 TaskLabel_1sub(:,2)./0.72 TaskLabel_1sub(:,3)]); %0.72 =  no of frames / second
        [temp1,idx_sort] = sort(TaskLabel_1sub_timestep(:,1));
        TaskLabel_1sub_timestep_sort = TaskLabel_1sub_timestep(idx_sort,:);
        
        TaskLabel_AllSubject_motor{subject} = TaskLabel_1sub_timestep_sort;
    end
    task_type_name = {'left foot','left hand','right foot','right hand','tongue','cue','null'};
    foldername = [main_folder,'/Sample Data/Motor Task/TaskLabel/'];
    save([foldername,'MotorTaskLabelAllSubject.mat'],'TaskLabel_AllSubject_motor','task_type_name');
