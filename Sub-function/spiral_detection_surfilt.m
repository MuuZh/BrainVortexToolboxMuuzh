...
analysis_path = [working_directory_path,'/Analysis'];
...
spiral_filt_nega_real = [];          %what
spiral_filt_pos_real = [];           %are
Vx_flowmap_norm_phase = [];          %each
Vy_flowmap_norm_phase = [];          %of 
spiral_size_nega_real= [];           %these
...
%%   Save normalised Phase-velocity fields  
           cd(analysis_path)
           filename = ['Spiral_detected_flowmaps_motor_task_LEFT_sub',num2str(subject),'.mat'];
           save([analysis_path,'/',filename],'Vx_flowmap_norm_phase','Vy_flowmap_norm_phase')
...
spiral_cav_pos_real=cell(size(spiral_filt_pos_real,1),size(spiral_filt_pos_real,2));
for ipatt = 1:size(spiral_filt_pos_real,1)
    for time = 1:size(spiral_filt_pos_real,2)
        spiral_mask = zeros(175,251);
        spiral_mask = spiral_filt_pos_real{ipatt,time}>0;
        if size(spiral_mask,1)==0
            continue 
        else
            dataMatrix =spiral_mask .* cav(:,:,time) ;
            dataMatrix(isnan(dataMatrix)) = 0;
            spiral_cav_pos_real{ipatt,time} = sparse(dataMatrix);
        end
    end
end
...
spiral_cav_nega_real=cell(size(spiral_filt_nega_real,1),size(spiral_filt_nega_real,2));
for ipatt = 1:size(spiral_filt_nega_real,1)
    for time = 1:size(spiral_filt_nega_real,2)
        spiral_mask = zeros(175,251);
        spiral_mask = spiral_filt_nega_real{ipatt,time}<0;
        if size(spiral_mask,1)==0
            continue 
        else
            dataMatrix =spiral_mask .* cav(:,:,time) ;
            dataMatrix(isnan(dataMatrix)) = 0;
            spiral_cav_nega_real{ipatt,time} = sparse(dataMatrix);
        end
    end
end
...
spiral_cav_pos_real_95perc_extend = [];
...
for ipatt = 1:size(spiral_filt_pos_real_95perc,1)
    ...
    if nansum(time_start(:)) ~= 0 && nansum(time_end(:)) ~= 0
        ...
        for t2 = 1:size(spiral_duration_start_end(:),1)
            ...
            spiral_cav_pos_real_95perc_extend{ipatt,t3} = spiral_cav_pos_real{ipatt,t3};
...
spiral_cav_nega_real_95perc_extend = [];
...
for ipatt = 1:size(spiral_filt_nega_real_95perc,1)
    ...
    if nansum(time_start(:)) ~= 0 && nansum(time_end(:)) ~= 0
        ...
        for t2 = 1:size(spiral_duration_start_end(:),1)
            ...
            spiral_cav_nega_real_95perc_extend{ipatt,t3} = spiral_cav_nega_real{ipatt,t3};
