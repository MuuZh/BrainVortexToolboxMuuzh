...
addpath(genpath([pwd]))
main_folder = pwd;
task_folder = 'data/DMT_data/PCB_group';
...
for iSub = subject:subject
    ...
    elseif flagRest == 0 % task data
        ...
        dataDir = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/Results/tfMRI_MOTOR_LR/','tfMRI_MOTOR_LR_Atlas.dtseries.nii']; 
        name = dir([main_folder,'/Sample Data/Language Task Original 100 sub/Data Pos']) ;        % position data of left/right hemisphere        
        if hemisphere == 1  
            posFile = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/fsaverage_LR32k/',name(iSub+2).name,'.L.flat.32k_fs_LR.surf.gii'] ; % Left brain
        elseif hemisphere == 2  
            posFile = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/fsaverage_LR32k/',name(iSub+2).name,'.R.flat.32k_fs_LR.surf.gii'] ; % Right brain
        end  
        ...
        % elseif flagTask == 4 % Motor task, original 100 subjects
        %     name = dir([main_folder,'/Sample Data/Motor Task/Raw Data']) ; % task data
        %     dataDir = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/Results/tfMRI_MOTOR_LR/','tfMRI_MOTOR_LR_Atlas.dtseries.nii']; 
        %     name = dir([main_folder,'/Sample Data/Motor Task/Data Pos']) ;        % position data of left/right hemisphere        
        %     if hemisphere == 1  
        %         posFile = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/fsaverage_LR32k/',name(iSub+2).name,'.L.flat.32k_fs_LR.surf.gii'] ; % Left brain
        %     elseif hemisphere == 2  
        %         posFile = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/fsaverage_LR32k/',name(iSub+2).name,'.R.flat.32k_fs_LR.surf.gii'] ; % Right brain
        %     end
        % end
        elseif flagTask == 4 % DMT data PCB group, original 100 subjects
            name = dir([main_folder,'/Sample Data/Motor Task/Raw Data']) ; % task data
            dataDir = [name(iSub+2).folder,'/',name(iSub+2).name,'/MNINonLinear/Results/tfMRI_MOTOR_LR/','tfMRI_MOTOR_LR_Atlas.dtseries.nii']; 
            name = dir([main_folder,'/Sample Data/Motor Task/Data Pos']) ;        % position data of left/right hemisphere        
            if hemisphere == 1  
    ...
    elseif flagSur == 1
       ...
       elseif flagRest == 0
          ...
          elseif flagTask == 3 % working memory task
              if flagSmooth == 1 
                  if hemisphere == 1 % left hemisphere
                     ...
                     DataOut_smooth = DataOut(:,:,:,1);  % smoothed
                     save(['Preprocessed_spatiotemporalbandpass_data_sur_WM_task_LEFT_sub',num2str(iSub),'.mat'],'DataOut_smooth')                 
                  elseif hemisphere == 2 % right hemisphere
                     folder_name = [main_folder,'/Sample Data/Working Memory Task/Preprocessed Data'];
                     cd(folder_name)
                     DataOut_unsmooth = DataOut(:,:,:,2);  % unsmoothed
          ...
          elseif flagTask == 4 % working memory task
              if flagSmooth == 1 
                  if hemisphere == 1 % left hemisphere
                     folder_name = [main_folder,'/Sample Data/Motor Task/Preprocessed Data'];
                     cd(folder_name)
